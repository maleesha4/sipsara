...
export async function GET() {
  try {
    ...
    const assignmentsResult = await query(
      `SELECT 
        a.id, a.title, a.description, a.due_date, a.closing_time, 
        a.max_score, a.status, a.created_at, a.updated_at, a.is_group,
        s.name as subject_name, g.grade_name, g.id as grade_id,
      ...
      GROUP BY a.id, a.is_group, s.name, g.grade_name, g.id
...
export async function POST(req) {
  try {
    ...
    const { title, description, gradeId, dueDate, closingTime, maxScore, isGroup } = body;

    if (!title || !gradeId || !dueDate || !closingTime) {
    ...
    const assignmentResult = await query(
      `INSERT INTO assignments (tutor_id, subject_id, grade_id, title, description, due_date, closing_time, max_score, is_group, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'active')
      RETURNING id, title, description, due_date, closing_time, max_score, is_group, created_at`,
      [tutorId, subjectId, gradeId, title, description || '', dueDate, closingTime, maxScore || 100, isGroup || false]
    );
    ...
    // Get enrolled students for this grade and subject
    const studentsResult = await query(
      `SELECT DISTINCT st.id FROM students st
      JOIN enrollments e ON st.id = e.student_id
      WHERE e.grade_id = $1 AND e.subject_id = $2 AND e.status = 'active'`,
      [gradeId, subjectId]
    );
